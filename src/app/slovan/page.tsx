...
const SLOVAN_SUBPAGES = new Set(['ticketing', 'crm', 'mobilni-aplikace', 'pripadove-studie', 'marketing', 'nabidka', 'partnersky-portal', 'ai-asistent', 'cashless', 'kiosks', 'sso', 'prodejni-system'])

const SLOVAN_NODES: DataNode[] = [
...
const ringProducts = SLOVAN_PRODUCTS.filter((p) => p.slug !== 'sso')
...
export default function Slovan() {
  const router = useRouter()

  const handleProductClick = (slug: string) => {
    track({ type: 'orbit_click', product: slug, ts: Date.now() })
    if (SLOVAN_SUBPAGES.has(slug)) router.push(`/slovan/${slug}`)
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 pointer-events-none" style={{
        ...
        background: 'radial-gradient(ellipse at center bottom, rgba(25,89,176,0.20) 0%, transparent 70%)',
        filter: 'blur(20px)',
      }} />
      ...
      <motion.nav
        ...
        style={{ zIndex: 50, background: 'rgba(0,6,20,0.90)', backdropFilter: 'blur(18px)', borderBottom: '1px solid rgba(255,255,255,0.07)' }}
        initial={{ opacity: 0, y: -16 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.4 }}
      >
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <button onClick={() => router.push('/slovan')} className="shrink-0 hover:opacity-70 transition-opacity cursor-pointer" title="Slovan ekosystém">
          <img src="/media/slovan-logo.png" alt="FC Slovan Liberec" style={{ height: 28, width: 'auto', objectFit: 'contain', filter: 'drop-shadow(0 1px 4px rgba(0,0,0,0.4))' }} />
        </button>
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto min-w-0" style={{ scrollbarWidth: 'none' }}>
          <button
            ...
            style={{ background: 'rgba(25,89,176,0.22)', color: '#6EA3E8', border: '1px solid rgba(25,89,176,0.5)' }}
          >
            SSO
          </button>
          {ringProducts.map((p) => (
        ...
        <button
          onClick={() => router.push('/slovan/analytics')}
          ...
          style={{ background: 'rgba(25,89,176,0.2)', color: 'rgba(110,163,232,0.85)', border: '1px solid rgba(25,89,176,0.35)' }}
        >
          Analytics
        </button>
      </motion.nav>
      ...
      >
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/media/slovan-logo.png" alt="FC Slovan Liberec" style={{ height: 40, width: 'auto', objectFit: 'contain', filter: 'drop-shadow(0 2px 10px rgba(0,0,0,0.5))' }} />
      ...
      <ExternalDataCloud nodes={SLOVAN_NODES} />
      ...
      >
        <OrbitDiagram
          ...
          products={SLOVAN_PRODUCTS}
          nodeBg="linear-gradient(135deg, rgba(0,20,60,0.97) 0%, rgba(0,8,30,0.97) 100%)"
        />
      </motion.div>

      {/* Nabídka banner */}
      <div className="relative w-full shrink-0 flex items-center justify-center px-6 py-3" style={{ zIndex: 50, background: 'rgba(0,4,15,0.97)', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        <motion.button
          onClick={() => router.push('/slovan/nabidka')}
