...
export default function SlovanAnalyticsLoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const res = await fetch('/api/slovan-analytics-auth', {
      method: 'POST',
  ...
  return (
    <main
      ...
      style={{ background: 'linear-gradient(160deg, #00102B 0%, #002450 50%, #003F8A 100%)' }}
    >
      <div className="relative z-10 flex flex-col items-center gap-7 w-full max-w-sm px-6">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src="/media/slovan-logo.png"
          alt="FC Slovan Liberec"
        ...
        <div className="flex flex-col items-center gap-1.5 text-center">
          ...
          <p className="text-xs font-semibold tracking-[0.22em] uppercase" style={{ color: 'rgba(255,255,255,0.35)' }}>
            FC Slovan Liberec · interní přístup
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 w-full">
          <input
            ...
            onFocus={e => {
              if (!error) e.currentTarget.style.border = '1px solid rgba(25,89,176,0.85)'
              e.currentTarget.style.boxShadow = '0 0 0 3px rgba(25,89,176,0.22)'
            }}
            onBlur={e => {
          ...
          <button
            ...
            style={{
              background: loading || !password
                ? 'rgba(25,89,176,0.5)'
                : 'linear-gradient(135deg, #1959B0 0%, #00306D 100%)',
              fontFamily: "'Mulish', sans-serif",
              letterSpacing: '0.06em',
              boxShadow: loading || !password ? 'none' : '0 4px 20px rgba(25,89,176,0.4)',
            }}
          >
