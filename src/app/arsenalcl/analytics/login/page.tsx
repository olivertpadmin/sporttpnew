...
export default function ArsenalCLAnalyticsLoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const res = await fetch('/api/arsenalcl-analytics-auth', {
      method: 'POST',
    ...
    if (res.ok) {
      router.push('/arsenalcl/analytics')
    } else {
      setError(true)
      setLoading(false)
    }
  ...
  return (
    <main
      ...
      style={{ background: 'linear-gradient(160deg, #060D1F 0%, #0D1E48 50%, #122F6E 100%)' }}
    >
      <div className="relative z-10 flex flex-col items-center gap-7 w-full max-w-sm px-6">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src="/media/arsenalcl-logo.png"
          alt="FK Arsenal Česká Lípa"
        ...
        <div className="flex flex-col items-center gap-1.5 text-center">
          ...
          <p className="text-xs font-semibold tracking-[0.22em] uppercase" style={{ color: 'rgba(255,255,255,0.35)' }}>
            FK Arsenal Česká Lípa · interní přístup
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 w-full">
          <input
            ...
            onFocus={e => {
              if (!error) e.currentTarget.style.border = '1px solid rgba(27,63,128,0.85)'
              e.currentTarget.style.boxShadow = '0 0 0 3px rgba(27,63,128,0.22)'
            }}
            onBlur={e => {
          ...
          <button
            ...
            style={{
              background: loading || !password
                ? 'rgba(27,63,128,0.5)'
                : 'linear-gradient(135deg, #1B3F80 0%, #0D2460 100%)',
              fontFamily: "'Mulish', sans-serif",
              letterSpacing: '0.06em',
              boxShadow: loading || !password ? 'none' : '0 4px 20px rgba(27,63,128,0.4)',
            }}
          >
