...
const ARSENALCL_PRODUCTS: Product[] = [
  {
    slug: 'sso',
    label: 'SSO',
    shortLabel: 'SSO',
...
const ARSENALCL_SUBPAGES = new Set(['ticketing', 'crm', 'mobilni-aplikace', 'pripadove-studie', 'marketing', 'nabidka', 'partnersky-portal', 'ai-asistent', 'cashless', 'kiosks', 'sso', 'prodejni-system'])

const ARSENALCL_NODES: DataNode[] = [
...
const ringProducts = ARSENALCL_PRODUCTS.filter((p) => p.slug !== 'sso')
...
export default function ArsenalCL() {
  const router = useRouter()

  const handleProductClick = (slug: string) => {
    track({ type: 'orbit_click', product: slug, ts: Date.now() })
    if (ARSENALCL_SUBPAGES.has(slug)) router.push(`/arsenalcl/${slug}`)
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 pointer-events-none" style={{
        ...
        background: 'radial-gradient(ellipse at center bottom, rgba(27,63,128,0.20) 0%, transparent 70%)',
        filter: 'blur(20px)',
      }} />
      ...
      <motion.nav
        ...
        style={{ zIndex: 50, background: 'rgba(4,10,32,0.90)', backdropFilter: 'blur(18px)', borderBottom: '1px solid rgba(255,255,255,0.07)' }}
        initial={{ opacity: 0, y: -16 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.4 }}
      >
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <button onClick={() => router.push('/arsenalcl')} className="shrink-0 hover:opacity-70 transition-opacity cursor-pointer" title="Arsenal ČL ekosystém">
          <img src="/media/arsenalcl-logo.png" alt="FK Arsenal Česká Lípa" style={{ height: 28, width: 'auto', objectFit: 'contain', filter: 'drop-shadow(0 1px 4px rgba(0,0,0,0.4))' }} />
        </button>
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto min-w-0" style={{ scrollbarWidth: 'none' }}>
          <button
            ...
            style={{ background: 'rgba(27,63,128,0.22)', color: '#6EA3E8', border: '1px solid rgba(27,63,128,0.5)' }}
          >
            SSO
          </button>
          {ringProducts.map((p) => (
        ...
        <button
          onClick={() => router.push('/arsenalcl/analytics')}
          ...
          style={{ background: 'rgba(27,63,128,0.2)', color: 'rgba(110,163,232,0.85)', border: '1px solid rgba(27,63,128,0.35)' }}
        >
          Analytics
        </button>
      </motion.nav>
      ...
      >
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/media/arsenalcl-logo.png" alt="FK Arsenal Česká Lípa" style={{ height: 40, width: 'auto', objectFit: 'contain', filter: 'drop-shadow(0 2px 10px rgba(0,0,0,0.5))' }} />
      ...
      <ExternalDataCloud nodes={ARSENALCL_NODES} />
      ...
      >
        <OrbitDiagram
          ...
          products={ARSENALCL_PRODUCTS}
          nodeBg="linear-gradient(135deg, rgba(6,15,50,0.97) 0%, rgba(5,12,40,0.97) 100%)"
        />
      </motion.div>

      {/* Nabídka banner */}
      <div className="relative w-full shrink-0 flex items-center justify-center px-6 py-3" style={{ zIndex: 50, background: 'rgba(4,8,30,0.97)', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        <motion.button
          onClick={() => router.push('/arsenalcl/nabidka')}
