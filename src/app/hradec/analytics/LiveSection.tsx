            ...
            <g key={i}>
              ...
              {barH > 0 && (
                <rect x={x} y={y} width={barW} height={barH} rx={3}
                  fill={isLast ? '#E8222B' : '#8B0000'} opacity={isLast ? 1 : 0.75} />
              )}
...
export default function LiveSection() {
  ...
  const fetch_ = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/hradec-analytics-live?minutes=${minutes}`)
      if (!res.ok) return
  ...
  return (
    <div className="rounded-xl p-6 space-y-5"
      style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(232,34,43,0.3)' }}>
        ...
        <div className="flex gap-1.5">
          {WINDOWS.map((w, i) => (
            <button key={w.minutes} onClick={() => setWindowIdx(i)}
              ...
              style={{
                background: i === windowIdx ? 'rgba(232,34,43,0.35)' : 'rgba(255,255,255,0.05)',
                color: i === windowIdx ? '#FF8E94' : 'rgba(255,255,255,0.4)',
                border: `1px solid ${i === windowIdx ? 'rgba(232,34,43,0.6)' : 'transparent'}`,
              }}>
              {w.label}
            </button>
          ))}
