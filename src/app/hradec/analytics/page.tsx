...
const HRADEC_PRODUCTS: Record<string, { label: string; color: string }> = {
...
const HRADEC_SLUGS = new Set(Object.keys(HRADEC_PRODUCTS))
...
export default async function HradecAnalyticsPage() {
    ...
    return (
      <main className="min-h-screen flex items-center justify-center px-8"
        ...
        <div className="rounded-xl p-8 max-w-lg w-full text-center"
          style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(232,34,43,0.4)' }}>
  ...
  const hradecPageViews = Object.fromEntries(
    Object.entries(pageViews ?? {}).filter(([p]) => p.startsWith('/hradec') && !p.startsWith('/hradec/analytics'))
  )
  const hradecOrbitClicks = Object.fromEntries(
    Object.entries(orbitClicks ?? {}).filter(([slug]) => HRADEC_SLUGS.has(slug))
  )
  const hradecSectionViews = Object.fromEntries(
    Object.entries(sectionViews ?? {}).filter(([key]) => HRADEC_SLUGS.has(key.split(':')[0]))
  )
  ...
  const totalPageViews = sum(hradecPageViews)
  const nabidkaViews = hradecPageViews['/hradec/nabidka'] ?? 0
  const totalOrbitClicks = sum(hradecOrbitClicks)
  const totalSectionViews = sum(hradecSectionViews)

  const topPages = sorted(hradecPageViews)
  const topOrbits = sorted(hradecOrbitClicks)
  const topSections = sorted(hradecSectionViews).slice(0, 10)
  ...
  const productColor = (slug: string) => HRADEC_PRODUCTS[slug]?.color ?? '#E8222B'
  const productLabel = (slug: string) => HRADEC_PRODUCTS[slug]?.label ?? slug

  const deviceColor: Record<string, string> = { Desktop: '#E8222B', Mobil: '#F59E0B', Tablet: '#10B981', Neznámý: '#ffffff30' }
  const referrerColor = '#6366F1'

  return (
    <main className="min-h-screen px-8 py-10"
      style={{ background: 'linear-gradient(160deg, #050505 0%, #161616 60%, #232323 100%)' }}>
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-white/40 text-sm mt-1">FC Hradec Králové · návštěvnost ekosystému</p>
          </div>
          <a href="/hradec" className="text-xs text-white/40 hover:text-white/70 transition-colors">
            ← Zpět na ekosystém
          </a>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard label="Zobrazení stránek" value={totalPageViews} accent="#E8222B" />
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="rounded-xl p-6 space-y-3"
            ...
            {topPages.map(([page, count]) => {
              const slug = page.replace(/^\/hradec\/?/, '') || 'home'
              return (
                <Bar key={page}
                  label={slug === 'home' || slug === '' ? 'Hradec home' : (productLabel(slug) || page)}
                  value={count} max={maxPages}
                  color={slug === 'home' || slug === '' ? '#E8222B' : productColor(slug)} />
              )
            })}
          </div>
        </div>
