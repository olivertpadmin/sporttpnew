...
const SPORT_PRODUCTS: Record<string, { label: string; color: string }> = {
...
const SPORT_SLUGS = new Set(Object.keys(SPORT_PRODUCTS))
...
export default async function SportAnalyticsPage() {
  ...
  if (redisError) {
    return (
      <main className="min-h-screen flex items-center justify-center px-8"
        style={{ background: 'linear-gradient(160deg, #0a0018 0%, #1a0840 50%, #0f0530 100%)' }}>
        <div className="rounded-xl p-8 max-w-lg w-full text-center"
          style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(147,51,234,0.4)' }}>
  ...
  const sportPageViews = Object.fromEntries(
    Object.entries(pageViews ?? {}).filter(([p]) => p.startsWith('/sport') && !p.startsWith('/sport/analytics'))
  )
  ...
  const sportOrbitClicks = Object.fromEntries(
    Object.entries(orbitClicks ?? {}).filter(([slug]) => SPORT_SLUGS.has(slug))
  )
  const sportSectionViews = Object.fromEntries(
    Object.entries(sectionViews ?? {}).filter(([key]) => SPORT_SLUGS.has(key.split(':')[0]))
  )
  ...
  const totalPageViews = sum(sportPageViews)
  const nabidkaViews = sportPageViews['/sport/nabidka'] ?? 0
  const totalOrbitClicks = sum(sportOrbitClicks)
  const totalSectionViews = sum(sportSectionViews)

  const topPages = sorted(sportPageViews)
  const topOrbits = sorted(sportOrbitClicks)
  const topSections = sorted(sportSectionViews).slice(0, 10)
  ...
  const productColor = (slug: string) => SPORT_PRODUCTS[slug]?.color ?? '#9333EA'
  const productLabel = (slug: string) => SPORT_PRODUCTS[slug]?.label ?? slug

  const deviceColor: Record<string, string> = { Desktop: '#9333EA', Mobil: '#F59E0B', Tablet: '#10B981', Neznámý: '#ffffff30' }
  const referrerColor = '#6366F1'

  return (
    <main className="min-h-screen px-8 py-10"
      style={{ background: 'linear-gradient(160deg, #0a0018 0%, #1a0840 60%, #0f0530 100%)' }}>
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-white/40 text-sm mt-1">PLG Sport · návštěvnost ekosystému</p>
          </div>
          <a href="/sport" className="text-xs text-white/40 hover:text-white/70 transition-colors">
            ← Zpět na ekosystém
          </a>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard label="Zobrazení stránek" value={totalPageViews} accent="#9333EA" />
            ...
            {topPages.map(([page, count]) => {
              ...
              return (
                <Bar key={page}
                  ...
                  color={slug === 'home' || slug === '' ? '#9333EA' : productColor(slug)} />
              )
            })}
          </div>
        </div>
