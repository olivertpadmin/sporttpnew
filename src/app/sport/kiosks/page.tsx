  ...
  return (
    <div>
      ...
      <div className="space-y-3">
        {steps.map((s) => (
          <div key={s.step} className="flex gap-5 p-5 rounded-2xl bg-[#EDE5FF] items-start">
            ...
            <div>
              <h3 className="font-bold text-[#0f0028] mb-1">{s.title}</h3>
...
export default function SportKiosksPage() {
    ...
    <div className="min-h-screen bg-[#F5F0FF]" style={{ '--product-color': ACCENT } as React.CSSProperties}>
      ...
      <section id="hardware" className="max-w-5xl mx-auto px-8 pt-24 pb-16">
        <motion.div {...fade(0.1)} className="flex flex-col md:flex-row gap-16 items-center">
          ...
          <div className="flex-1">
            ...
            <h2 className="text-4xl md:text-5xl font-black text-[#0f0028] mt-3 mb-5 leading-tight"
              style={{ fontFamily: "'Panel Sans', sans-serif" }}>
              AF910 Kiosk
            </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
              {SPECS.map((s) => (
                <div key={s.label} className="flex items-start gap-3 py-2" style={{ borderBottom: '1px solid #0f002810' }}>
                  <span className="text-xs font-semibold text-[#3A3342]/40 uppercase tracking-wide w-28 shrink-0 pt-0.5">{s.label}</span>
                  <span className="text-sm font-medium text-[#0f0028]">{s.value}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section id="kiosk-apps" className="max-w-5xl mx-auto px-8 pb-20">
        <motion.div {...fade(0.15)}>
          <h2 className="text-2xl font-bold text-[#0f0028] mb-2">Aplikace v kiosku</h2>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            {/* Ticketing App */}
            <div className="rounded-3xl overflow-hidden border border-[#9333EA]/15 bg-[#EDE5FF]">
              <div className="px-7 py-6 flex items-center gap-4" style={{ borderBottom: '1px solid #0f00280A' }}>
                <div className="w-11 h-11 rounded-2xl flex items-center justify-center text-xl"
                  style={{ background: `${ACCENT}18` }}>
                  🎟
                </div>
                <div>
                  <h3 className="font-bold text-[#0f0028] text-base">Ticketing aplikace</h3>
            ...
            <div className="rounded-3xl overflow-hidden border border-[#9333EA]/15 bg-[#EDE5FF]">
              <div className="px-7 py-6 flex items-center gap-4" style={{ borderBottom: '1px solid #0f00280A' }}>
                ...
                <div>
                  <h3 className="font-bold text-[#0f0028] text-base">Cashless aplikace</h3>
      ...
      <div className="max-w-5xl mx-auto px-8 pb-24">
        <motion.section id="how-it-works" {...fade(0.2)} className="mb-14">
          <h2 className="text-2xl font-bold text-[#0f0028] mb-6">Jak to funguje</h2>
          <HowItWorksTabs />
        </motion.section>

        <motion.section id="where-we-deploy" {...fade(0.3)}>
          <h2 className="text-2xl font-bold text-[#0f0028] mb-6">Kde kiosky nasazujeme</h2>
          <div className="grid md:grid-cols-3 gap-4">
            {USE_CASES.map((u) => (
              <div key={u.title} className="p-6 rounded-2xl border border-[#9333EA]/15 hover:border-[#3B82F6]/30 transition-colors text-center">
                <span className="text-4xl mb-3 block">{u.icon}</span>
                <h3 className="font-bold text-[#0f0028] mb-2">{u.title}</h3>
