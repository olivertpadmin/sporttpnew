...
const KOMETA_PRODUCTS: Record<string, { label: string; color: string }> = {
...
const KOMETA_SLUGS = new Set(Object.keys(KOMETA_PRODUCTS))
...
export default async function KometaAnalyticsPage() {
    ...
    return (
      <main className="min-h-screen flex items-center justify-center px-8"
        ...
        <div className="rounded-xl p-8 max-w-lg w-full text-center"
          style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(0,102,172,0.3)' }}>
  ...
  const kometaPageViews = Object.fromEntries(
    Object.entries(pageViews ?? {}).filter(([p]) => p.startsWith('/kometa') && !p.startsWith('/kometa/analytics'))
  )
  const kometaOrbitClicks = Object.fromEntries(
    Object.entries(orbitClicks ?? {}).filter(([slug]) => KOMETA_SLUGS.has(slug))
  )
  const kometaSectionViews = Object.fromEntries(
    Object.entries(sectionViews ?? {}).filter(([key]) => KOMETA_SLUGS.has(key.split(':')[0]))
  )
  ...
  const totalPageViews = sum(kometaPageViews)
  const nabidkaViews = kometaPageViews['/kometa/nabidka'] ?? 0
  const totalOrbitClicks = sum(kometaOrbitClicks)
  const totalSectionViews = sum(kometaSectionViews)

  const topPages = sorted(kometaPageViews)
  const topOrbits = sorted(kometaOrbitClicks)
  const topSections = sorted(kometaSectionViews).slice(0, 10)
  ...
  const productColor = (slug: string) => KOMETA_PRODUCTS[slug]?.color ?? '#0066AC'
  const productLabel = (slug: string) => KOMETA_PRODUCTS[slug]?.label ?? slug

  const deviceColor: Record<string, string> = { Desktop: '#0066AC', Mobil: '#F59E0B', Tablet: '#10B981', Neznámý: '#ffffff30' }
  const referrerColor = '#6366F1'

  return (
    <main className="min-h-screen px-8 py-10"
      style={{ background: 'linear-gradient(160deg, #001228 0%, #002450 60%, #003a70 100%)' }}>
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-white/40 text-sm mt-1">HC Kometa Brno · návštěvnost ekosystému</p>
          </div>
          <a href="/kometa" className="text-xs text-white/40 hover:text-white/70 transition-colors">
            ← Zpět na ekosystém
          </a>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard label="Zobrazení stránek" value={totalPageViews} accent="#0066AC" />
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="rounded-xl p-6 space-y-3"
            ...
            {topPages.map(([page, count]) => {
              const slug = page.replace(/^\/kometa\/?/, '') || 'home'
              return (
                <Bar key={page}
                  label={slug === 'home' || slug === '' ? 'Kometa home' : (productLabel(slug) || page)}
                  value={count} max={maxPages}
                  color={slug === 'home' || slug === '' ? '#0066AC' : productColor(slug)} />
              )
            })}
          </div>
        </div>
